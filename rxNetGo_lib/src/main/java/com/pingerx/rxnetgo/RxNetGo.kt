...
import com.pingerx.rxnetgo.interceptor.DynamicTimeoutInterceptor
...
import com.pingerx.rxnetgo.request.DeleteRequest
import com.pingerx.rxnetgo.request.GetRequest
import com.pingerx.rxnetgo.request.PostRequest
import com.pingerx.rxnetgo.request.PutRequest
...
class RxNetGo {
    ...
    companion object {
        ...
        const val DEFAULT_MILLISECONDS: Long = 10000      // 默认的超时时间,5秒
        const val FILE_UPLOAD_MILLISECONDS:Int = 10000   // 文件上传超时时间为10秒
    ...
    private fun initNetGo() {
        ...
        //超时时间(上传接口与普通接口超时不同)
        val dynamicTimeoutInterceptor = DynamicTimeoutInterceptor()
    ...
    /**
     * put请求，需要先初始化对应的Service
     */
    fun <T> put(url: String = ""): PutRequest<T> {
        return PutRequest(url, mService, null)
    }

    /**
     * post请求，由用户自定义Service
     */
    fun <T> put(flowable: Flowable<T>): PutRequest<T> {
        return PutRequest("", null, flowable)
    }


    /**
     *delete请求，需要先初始化对应的Service
     */
    fun <T> delete(url: String = ""): DeleteRequest<T> {
        return DeleteRequest(url, mService, null)
    }

    /**
     * delete请求，由用户自定义Service
     */
    fun <T> delete(flowable: Flowable<T>): DeleteRequest<T> {
        return DeleteRequest("", null, flowable)
    }
