...
import com.pingerx.rxnetgo.utils.EncryptUtil
...
abstract class Request<T>(
    ...
    init {
        ...
        val publicHeaders = mRxNetGo.getCommonHeaders("public")
        val userHeaders = mRxNetGo.getCommonHeaders("user")
        if (httpParams?.getUrlParams()?.isNotEmpty() == true)
            params(httpParams)
        if (httpHeaders?.getHeaderParams()?.isNotEmpty() == true)
            headers(httpHeaders)
        if (publicHeaders?.getHeaderParams()?.isNotEmpty() == true)
            headers(publicHeaders)
        if (userHeaders?.getHeaderParams()?.isNotEmpty() == true)
            headers(userHeaders)
        // 添加缓存模式
...
//        if (!noDefault) {
//            mParams.put("appId", "3")
//            jsonParamsMap.putAll(mParams.getUrlParams())
//            val jsonString = EncryptUtil.toJsonString(jsonParamsMap)
//            val encrypt = EncryptUtil.getEncrypt(jsonString)
//            mParams.put("sign", encrypt)
//            jsonParamsMap.clear()
//        }
        return mParams
    }
     ...
     * 获取Json请求参数对象
     */
    private var jsonParamsMap: LinkedHashMap<String, Any> = LinkedHashMap()

    /**
     * 去除默认参数
     */
    private var noDefault = false

    fun setNoDefault(): Request<T> {
        this.noDefault = true
        return this
    }
    ...
    fun flowable(): Flowable<T> {
        return RxNetHelper.rxAsync(this, null)
    }
    ...
    fun query(query: String, params: String): Request<T> {
        val reg = "{$query}"
        url = url.replace(reg, params)
        return this
    }
    ...
    fun jsonParams(key: String, value: String): Request<T> {
        this.jsonParamsMap.put(key, value)
        return this
    }

    fun params(key: String, value: String?, deFaultValue: String = "", isNullAdded: Boolean): Request<T> {
        if (TextUtils.isEmpty(value)) {
            if (isNullAdded) {
                this.mParams.put(key, deFaultValue)
            }
        } else {
            this.mParams.put(key, value)
        }
    ...
    fun params(key: String, value: Int?, deFaultValue: Int = 0, isNullAdded: Boolean): Request<T> {
        if (value == null) {
            if (isNullAdded) {
                this.mParams.put(key, deFaultValue)
            }
    ...
    fun params(key: String, value: Boolean?, deFaultValue: Boolean = false, isNullAdded: Boolean): Request<T> {
        if (value == null) {
            if (isNullAdded) {
                this.mParams.put(key, deFaultValue)
            }
    ...
    fun params(key: String, value: Float?, deFaultValue: Float = 0f, isNullAdded: Boolean): Request<T> {
        if (value == null) {
            if (isNullAdded) {
                this.mParams.put(key, deFaultValue)
            }
    ...
    fun jsonParams(key: String, value: Long): Request<T> {
        this.jsonParamsMap.put(key, value)
        return this
    }

    fun params(key: String, value: Long?, deFaultValue: Long = 0L, isNullAdded: Boolean): Request<T> {
        if (value == null) {
            if (isNullAdded) {
                this.mParams.put(key, deFaultValue)
            }
