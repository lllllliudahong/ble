...
abstract class BodyRequest<T>(url: String, service: ApiService?, flowable: Flowable<T>?) : Request<T>(url, service, flowable), IBody<Request<T>> {
    ...
    override fun generateRequestBody(): RequestBody? {
        ...
        return when {
            ...
            // 上传一个文件
            file != null && mediaType != null -> {
                val builder = MultipartBody.Builder()
                if (isMultipart){
                    builder.setType(MultipartBody.FORM)
                }
                builder.addPart(MultipartBody.Part.createFormData("file", file?.name, RequestBody.create(mediaType, file!!)))
                builder.build()
            }
    ...
    override fun upJson(json: String): Request<T> {
        ...
        val jsonParamsMap = JsonUtils.jsonToMap(JSONObject(json))
        jsonParamsMap.forEach {
            jsonParams(it.key,it.value)
        }
        return this
    }
    ...
    override fun upJson(jsonObject: JSONObject): Request<T> {
        ...
        this.mediaType = HttpParams.MEDIA_TYPE_JSON
        val jsonParamsMap = JsonUtils.jsonToMap(jsonObject)
        jsonParamsMap.forEach {
            jsonParams(it.key,it.value)
        }
        return this
